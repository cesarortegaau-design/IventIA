...
import {
  MessageOutlined, SendOutlined, CloseOutlined, ArrowLeftOutlined,
  CustomerServiceOutlined, PaperClipOutlined, FileOutlined, LoadingOutlined,
...
export default function ChatWidget({ isMobile = false }: { isMobile?: boolean }) {
  ...
  const { data: conversations = [] } = useQuery({
    queryKey: ['supplier-chat', 'conversations'],
  ...
  const { data: conversation, isLoading: loadingConv } = useQuery({
    queryKey: ['supplier-chat', 'conversation', selectedId],
  ...
  const sendMut = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      qc.invalidateQueries({ queryKey: ['supplier-chat'] })
    },
  })
    ...
    <div style={{ position: 'fixed', bottom: floatBottom, right: floatRight, zIndex: 1000 }}>
      ...
      {!open && (
        <Badge count={unread} size="small">
          <Button
            ...
            style={{ width: 56, height: 56, background: BLUE, borderColor: BLUE, boxShadow: '0 4px 16px rgba(3,105,161,0.4)' }}
          />
        </Badge>
      )}
      ...
      {open && (
        <div style={
          ...
          {/* Header */}
          <div style={{ background: BLUE, padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {view === 'list' && (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                    ...
                    return (
                      ...
                      >
                        <Badge count={conv.unreadPortal} size="small">
                          <Avatar icon={<CustomerServiceOutlined />} style={{ background: BLUE }} />
                        </Badge>
              ...
              <div style={{ padding: 12, borderTop: '1px solid #f1f5f9' }}>
                {!showNew ? (
                  <Button block type="primary" onClick={() => setShowNew(true)} style={{ background: BLUE, borderColor: BLUE }}>
                    + Nueva consulta
                  </Button>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      ...
                      <Button size="small" type="primary" onClick={send} loading={sendMut.isPending} disabled={!text.trim()}
                        style={{ flex: 1, background: BLUE, borderColor: BLUE }}>Enviar</Button>
                    </div>
                  </div>
                )}
              </div>
          ...
          {view === 'conv' && (
            <>
              <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px', background: '#e0f2fe22' }}>
                    ...
                    return (
                      <div key={msg.id} style={{ display: 'flex', justifyContent: isAdmin ? 'flex-start' : 'flex-end', marginBottom: 10 }}>
                        <div style={{
                          ...
                          background: isAdmin ? BLUE : '#f1f5f9',
                          ...
                          {msg.fileUrl && (
                            msg.fileUrl.match(/\.(jpg|jpeg|png|gif|webp)$/i)
                              ...
                              : <a href={`${apiBase}${msg.fileUrl}`} target="_blank" rel="noreferrer"
                                  style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: msg.content ? 4 : 0, color: isAdmin ? '#bae6fd' : BLUE, fontSize: 12 }}>
              ...
              <div style={{ padding: '10px 12px', borderTop: '1px solid #f1f5f9' }}>
                {pendingFile && (
                  <div style={{ marginBottom: 6, padding: '4px 8px', background: '#e0f2fe', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  ...
                  <Button
                    ...
                    style={{ background: BLUE, borderColor: BLUE, flexShrink: 0 }}
                  />
                </div>
              </div>
            </>
