...
// ── Create Order from Spaces Modal ───────────────────────────────────────────
function CreateOrderFromSpacesModal({
  open, onClose, onSuccess, eventId, events,
}: {
  open: boolean; onClose: () => void; onSuccess: () => void;
  eventId: string; events: any[];
}) {
  const { message } = App.useApp()
  const [form] = Form.useForm()
  ...
  const today = dayjs()
  const tier = earlyCutoff && today.isBefore(dayjs(earlyCutoff)) ? 'early'
  ...
  const eventName = events.find((e: any) => e.id === eventId)?.name ?? ''
    ...
    >
      ...
      {selectedPriceListId && (
        <div style={{ marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {!earlyCutoff && !normalCutoff && <Text style={{ fontSize: 11, color: '#94a3b8' }}>Lista sin fecha de corte — usando tarifa tardía</Text>}
        </div>
      )}
      ...
      {spacesLoading ? (
        ...
        <>
          ...
          <div style={{ border: '1px solid #e2e8f0', borderRadius: 8, overflow: 'hidden', marginBottom: 12 }}>
            {/* Table header */}
            <div style={{
              display: 'grid', gridTemplateColumns: '32px 1fr 160px 90px 70px 90px',
            ...
            {spaces.map((space: any) => {
              ...
              return (
                <div key={space.id} style={{
                  ...
                  <input type="checkbox" checked={checked} style={{ cursor: 'pointer', width: 15, height: 15 }}
                    onChange={e => {
                  ...
                  <div>
                    ...
                    {space.resource?.code && <Text style={{ fontSize: 11, color: '#94a3b8', marginLeft: 6 }}>{space.resource.code}</Text>}
                    <div style={{ marginTop: 2 }}>
                      <span style={{
                        fontSize: 11, borderRadius: 3, padding: '1px 6px',
                        background: phaseSt?.background, border: `1px solid ${phaseSt?.color}`, color: phaseSt?.color,
                      }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              {selectedPriceListId && <div style={{ fontSize: 11, color: '#94a3b8' }}>Tarifa {tierLabel}</div>}
            </div>
          </div>
        </>
      )}
